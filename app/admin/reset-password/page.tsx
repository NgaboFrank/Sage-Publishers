...
export default function ResetPasswordPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setLoading(true)
    try {
      const res = await fetch('/api/admin/password-reset/confirm', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token, password }) })
    ...
    } catch (e) { setError(e instanceof Error ? e.message : 'Unable to reset the password.') }
  ...
  return <main className="min-h-screen bg-[#f5f7f6] flex items-center justify-center px-5">
    <form onSubmit={submit} className="w-full max-w-md rounded-3xl bg-white p-8 shadow-xl border border-black/5">
      <div className="mb-8"><div className="text-2xl font-bold text-[#14532d]">Sage Publishers</div><p className="mt-2 text-sm text-gray-500">Create a new administrator password</p></div>
