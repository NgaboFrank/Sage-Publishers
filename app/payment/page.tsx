    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_50%_0%,_rgba(82,200,120,0.10),_transparent_35%)] px-5 py-20 md:px-8 md:py-28">
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mb-10 text-center md:mb-12">
          ...
          <p className="mx-auto mt-4 max-w-xl leading-relaxed text-muted-foreground">Choose your book, then choose how you want to pay.</p>
        </div>
        ...
        <form onSubmit={handleCardPayment} className="rounded-[2rem] border border-border bg-card/95 p-7 shadow-2xl shadow-forest/10 backdrop-blur md:p-10">
          <div className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <button type="button" onClick={()=>setPaymentMethod('bk')} className={`rounded-2xl border p-5 text-left transition-all ${paymentMethod==='bk' ? 'border-emerald bg-emerald/10 shadow-md shadow-emerald/10' : 'border-border bg-background hover:border-emerald/50'}`}>
                <div className="flex items-center justify-between gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-emerald/15 text-emerald"><CreditCard className="h-5 w-5" /></div>{paymentMethod==='bk' && <CheckCircle2 className="h-5 w-5 text-emerald" />}</div>
                <h2 className="mt-4 font-semibold text-forest">BK Pay</h2><p className="mt-1 text-sm text-muted-foreground">Pay using BK Pay code</p>
              </button>
              <button type="button" onClick={()=>setPaymentMethod('card')} className={`rounded-2xl border p-5 text-left transition-all ${paymentMethod==='card' ? 'border-emerald bg-emerald/10 shadow-md shadow-emerald/10' : 'border-border bg-background hover:border-emerald/50'}`}>
                <div className="flex items-center justify-between gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-muted text-forest"><CreditCard className="h-5 w-5" /></div>{paymentMethod==='card' && <CheckCircle2 className="h-5 w-5 text-emerald" />}</div>
                <h2 className="mt-4 font-semibold text-forest">Visa / Card</h2><p className="mt-1 text-sm text-muted-foreground">Pay securely by card</p>
              </button>
            </div>

            {paymentMethod === 'bk' && <div className="rounded-2xl border border-emerald/20 bg-emerald/5 p-5">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-moss">BK Pay Code</p>
              <div className="mt-3 flex items-center justify-between gap-3 rounded-xl border border-border bg-card p-4"><p className="text-2xl font-bold tracking-wider text-forest">{BK_PAY_CODE}</p><button type="button" onClick={copyBkPayCode} className="inline-flex shrink-0 items-center gap-2 rounded-xl bg-forest px-4 py-2.5 text-sm font-semibold text-white transition hover:opacity-90"><Copy className="h-4 w-4" />{copied ? 'Copied' : 'Copy'}</button></div>
              <p className="mt-3 text-sm leading-6 text-muted-foreground">Use this code when paying Sage Publishers with BK Pay, then keep your payment confirmation.</p>
            </div>}

            {paymentMethod === 'card' && <div className="rounded-2xl border border-border bg-muted/40 p-5"><p className="font-semibold text-forest">Visa / Card payment</p><p className="mt-1 text-sm text-muted-foreground">You will continue to the secure card checkout after entering your details below.</p></div>}
