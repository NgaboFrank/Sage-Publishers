...
export async function POST(request: Request) {
  try {
    ...
    if (!admin || admin.password_hash !== hashPassword(password)) return NextResponse.json({ error: 'Invalid email or password.' }, { status: 401 })
    const response = NextResponse.json({ ok: true, admin: { id: admin.id, email: admin.email, name: admin.name } })
    response.cookies.set('sage_admin', makeAdminToken(admin.id), { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax', path: '/', maxAge: 60 * 60 * 24 * 7 })
