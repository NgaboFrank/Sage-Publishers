    ...
    <section ref={ref} id="top" className="relative isolate flex min-h-[92vh] items-center overflow-hidden bg-forest-deep pt-28 text-cream md:min-h-screen md:pt-32">
      ...
      <div className="relative z-10 mx-auto grid w-full max-w-7xl items-center gap-14 px-5 pb-20 md:px-8 lg:grid-cols-[1fr_0.9fr] lg:gap-20 lg:pb-24">
        <motion.div style={{ y: textY }} className="max-w-2xl text-center lg:text-left">
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="inline-flex items-center gap-2 rounded-full border border-emerald/25 bg-emerald/10 px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-emerald">
            <Sparkles className="h-3 w-3" /> A Sage Publishers original
          </motion.div>
          <motion.h1 initial={{ opacity: 0, y: 26 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.85, delay: 0.1, ease: [0.22, 1, 0.36, 1] }} className="mt-5 font-serif text-4xl font-semibold leading-[0.98] tracking-tight sm:text-5xl md:text-5xl xl:text-[4rem]">
            The Breeze
            <br />
          ...
          <motion.p initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.75, delay: 0.38 }} className="mx-auto mt-5 max-w-lg text-[13px] leading-relaxed text-cream/70 sm:text-sm lg:mx-0">
          ...
          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.75, delay: 0.5 }} className="mt-8 flex flex-col items-center gap-3 sm:flex-row lg:items-start">
          ...
          <div className="mt-7 flex flex-wrap items-center justify-center gap-x-5 gap-y-2 text-[11px] text-cream/50 lg:justify-start">
